...
>
> Placeholders are written in backticks (`` `<title>` ``) — a bare `<title>` is parsed as an HTML
> tag: every renderer drops it, and `<title>` in particular swallows the rest of the document.
